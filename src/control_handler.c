...
extern void recv_file(int,char*);
...
{
  int fdaccept,caddr_len;
  struct sockaddr_in data_controller_addr;

  caddr_len = sizeof(data_controller_addr);
  fdaccept = accept(sock_index,(struct sockaddr*)&data_controller_addr, &caddr_len);
  if(fdaccept<0)
      ERROR("Data Accept Failed");
  data_connection  = malloc(sizeof(struct DataConn));
...
{
  char *data_header;
  data_header = (char*) malloc (1036);
  if(recvALL(sock_index,data_header,1036)<0)
  {
    // printf("No data\n");
    free(data_header);
    return false;
  }
  recv_file(sock_index,data_header);
  return true;
}
...
bool control_recv_hook(int sock_index)
{
    ...
    switch(control_code){
        ...
        case 2: send_update(sock_index,1);
                break;

        case 3: update_table(sock_index,cntrl_payload);
                break;

        case 5: send_file(sock_index,payload_len,cntrl_payload);
                break;
        /* .......
            .........
           .......
